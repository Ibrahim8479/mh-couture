...
<?php
/**
 * Fonctions utilitaires globales - MH Couture
 * Version moderne, robuste et sécurisée
 */

// Définir les headers JSON pour les API
function setJSONHeaders() {
    header('Content-Type: application/json; charset=utf-8');
    header('Access-Control-Allow-Origin: *');
    header('Access-Control-Allow-Methods: POST, GET, OPTIONS, PUT, DELETE');
...
// Valider un téléphone Niger (+227...)
function validatePhone($phone) {
    $phone = preg_replace('/[^0-9+]/', '', $phone);
    return (strpos($phone, '+227') === 0 && strlen($phone) >= 12);
}
...
// Vérifier le token utilisateur et retourner l'utilisateur
function getUserIdFromToken($token) {
    require_once __DIR__ . '/../config/database.php';
    $conn = getDBConnection();
    if (!$conn) return null;
    try {
        $stmt = $conn->prepare("SELECT id, email, first_name, last_name, is_admin FROM users WHERE token = ? AND is_active = 1");
        $stmt->execute([$token]);
        $user = $stmt->fetch();
        return $user ? $user : null;
    } catch (PDOException $e) {
...
// Gérer l'upload de fichiers (images produits)
function uploadFile($file, $targetDir = '../../uploads/products/') {
    if (!isset($file) || $file['error'] !== UPLOAD_ERR_OK) {
        return ['success' => false, 'message' => "Erreur lors de l'upload"];
    }
...
// Envoyer un email (log en dev, mail réel en prod)
function sendEmail($to, $subject, $message, $from = 'info@mhcouture.com') {
    $headers = "From: $from\r\n";
    $headers .= "Reply-To: $from\r\n";
    $headers .= "Content-Type: text/plain; charset=UTF-8\r\n";
